...
-- Создаем таблички
 ...
 VALUES
  ...
  ('Saint_Petersburg', 'Vorkuta'),
  ('Saint_Petersburg', 'Moscow'),
  ('Yelets', 'Vorkuta'),
  ('Moscow', 'Chita'),
  ('Chita', 'Saint_Petersburg'),
  ('Moscow', 'Yelets'),
  ('Chita', 'Vorkuta')
  ;
 ...
 CREATE TABLE cities (
  label varchar(100),
  name varchar(100)
  ) COMMENT = 'Соответствие англ.названий русским';
...
select
flights.id,
t1.name as R_city_from, 
t2.name as R_city_to
from flights 
join cities as t1
on flights.from_city = t1.label
join cities as t2
on flights.to_city = t2.label
order by flights.id;
